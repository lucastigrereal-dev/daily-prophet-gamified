...
export default function WorkflowDetailPage() {
  ...
  useEffect(() => { if (params.id) loadWorkflow(params.id as string); }, [params.id]);

  async function loadWorkflow(id: string) {
    try {
      setLoading(true);
      const { data, error } = await supabase.from('postpack_workflow').select('*').eq('id', id).single();
  ...
  async function avancarFase() {
    if (!workflow) return;
    const faseAtual = parseInt(workflow.status.replace('fase_', ''));
    if (isNaN(faseAtual) || faseAtual >= 5) return;

    try {
      setUpdating(true);
      const prox = faseAtual + 1;
      const { error } = await supabase.from('postpack_workflow').update({
  ...
  async function concluir() {
    ...
    try {
      ...
      const { error } = await supabase.from('postpack_workflow').update({
        ...
        fase_5_completed_at: new Date().toISOString(),
  ...
  async function excluir() {
    ...
    try {
      setUpdating(true);
      const { error } = await supabase.from('postpack_workflow').delete().eq('id', workflow.id);
      if (error) throw error;
  ...
  const fases = [
    ...
    { num: 2, key: 'fase_2', status: workflow.fase_2_status, completed: workflow.fase_2_completed_at },
    { num: 3, key: 'fase_3', status: workflow.fase_3_status, completed: workflow.fase_3_completed_at },
    { num: 4, key: 'fase_4', status: workflow.fase_4_status, completed: workflow.fase_4_completed_at },
    { num: 5, key: 'fase_5', status: workflow.fase_5_status, completed: workflow.fase_5_completed_at },
  ];
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6">
          ...
          <div className="flex gap-3">
            {workflow.status !== 'concluido' && workflow.status !== 'fase_5' && (
              <button onClick={avancarFase} disabled={updating} className="flex-1 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-gray-400">
                {updating ? 'Atualizando...' : 'Avancar Fase'}
              </button>
            )}
            {workflow.status === 'fase_5' && (
              <button onClick={concluir} disabled={updating} className="flex-1 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:bg-gray-400">
